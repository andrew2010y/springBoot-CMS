...
layui.config({
    ...
    var form = layui.form;
    var tableCheckbox = []; //table表格中的复选框
    var ratifyAllDataCheckbox = [];
    ...
    var createTable ={  //table封装
        ...
        dataArr:[          	 //表头内容
            ...
            {field:'vc2sdkusername',title: '提现用户名'},
        ...
        getData:function (type) { //获取查询条件
            var that = this;
            var data = "";
            this.vc2sdkusername = $("#userID").val(); //获取用户ID
            ...
            this.requestData = {
                "vc2sdkusername":that.vc2sdkusername,
            ...
            if (type===1) {
                ...
                data = "?vc2sdkusername="+this.vc2sdkusername+"&datcreateStart="+this.datcreateStart+"&datcreateEnd="+this.datcreateEnd+"&numtransactionnumMin="+this.numtransactionnumMin+"&numtransactionnumMax="+this.numtransactionnumMax+"&vc2status="+this.vc2status;
                return data;
            ...
            this.tableOptions = {
                ...
                limit:20,
                    ...
                    if (type===1) { //这是代表未审核状态
                        ...
                        form.render();
                        $("thead th .laytable-cell-checkbox .layui-unselect").remove();
    ...
    table.on('checkbox(test)', function(obj){ //table 复选按钮  tableCheckbox
        var tbodyCheckbox = $('tbody input[type="checkbox"]');


        // console.log("obj",obj.data.numthirdbindingcashid,obj.checked);

        if( obj.data.numthirdbindingcashid ){ //不是table表格中全选按钮
            var objCheckbox = {
                id:obj.data.numthirdbindingcashid,
                boolean:obj.checked,
            }

            if( tableCheckbox.length>0 ){
                for(var i=0;i<tableCheckbox.length;i++){
                    // console.log("已经存在",objCheckbox,tableCheckbox[i]);
                    if( tableCheckbox[i].id === objCheckbox.id){
                        tableCheckbox[i].boolean = objCheckbox.boolean;
                        checkSwitch = false;
                        break;
                    }
                }
            ...
            if(checkSwitch){
                tableCheckbox.push(objCheckbox);
            }

        }

        // console.log("table success",tableCheckbox);
    ...
    form.on('checkbox(formAllCheck)', function(data){ //全选按钮
        ...
        if (checkBoolean) {
            ...
            createTable.data[0].forEach(function (ele, i) {
                ...
                objCheckBoxData = {
                    id:ele.numthirdbindingcashid,
                    boolean:true,
                }

                ratifyAllDataCheckbox.push(objCheckBoxData);

            });

            tableCheckbox = ratifyAllDataCheckbox;

            // console.log("ratifyAllDataCheckbox",ratifyAllDataCheckbox)
    ...
    form.on('submit(formRatify)', function(data){ //一键批准
        console.log("一键批准");
        if(tableCheckbox.length==0 && ratifyAllDataCheckbox.length==0 ){
            return;
        }
        layer.open({
            title:'一键批准',
            ...
            btn1: function(index, layero){
                ...
                if(tableCheckbox.length>0){ //这是选一个或者多个，非全选
                    ratifyAllData = "";
                    tableCheckbox.forEach(function (ele,i) {
                        console.log("ele",ele);
                        if(ele.boolean){ //这是选中状态
                            ratifyAllData += '&numthirdbindingcashid=' + ele.id;
                        }
                    })
                }
                ...
                tableCheckbox = [];
                ratifyAllDataCheckbox = [];
                console.log( "yeS框",ratifyAllData );
                ...
                // ajax('/ratify.json',"get",ratifyAllData);
                //重新请求 未审核数据
                createTable.init('/alipay-withdrawal/list',1);
                layer.close(index);
            },
